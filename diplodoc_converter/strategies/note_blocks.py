# diplodoc_converter/strategies/note_blocks.py
import re
from .base import TransformationStrategy

class TwoColumnTableNoteStrategy(TransformationStrategy):
    """
        ...
        while i < len(lines):
            ...
            if re.match(r'^\+[-]+\+[-]+\+', lines[i].strip()):
                ...
                for row in cell_rows:
                    ...
                    right = re.sub(r'(?<!!)\|', ' ', right)  # заменяем |, если перед ним нет '!'
...
class HorizontalRuleNoteStrategy(TransformationStrategy):
    """
    Преобразует блоки вида:
    ------------------------------------------------------------------------------
    ![image.png](path)   ***Текст заметки***
    ------------------------------------------------------------------------------
    в {% note info "Test2" %}...{% endnote %} без изображения.
    """
    def transform(self, content: str) -> str:
        lines = content.splitlines(keepends=True)
        new_lines = []
