...
def convert_odt_to_diplodoc(odt_path, output_dir):
    ...
    for idx, section in enumerate(sections):
        ...
        root_toc_items.append({
            "name": raw_title,
            "href": f"{folder_name}/toc.yaml"
        })

    # Шаг 5: Создаем корневой toc.yaml в output_dir
    root_toc = {
        "title": "Документация",
        "items": root_toc_items
    }
    ...
    if os.path.exists(temp_media_dir): shutil.rmtree(temp_media_dir)
