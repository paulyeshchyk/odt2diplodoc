...
# ------------------------------------------------------------
# Постобработка ODT (улучшенные стратегии)
# ------------------------------------------------------------
...
class OdtPostProcessor:
    ...
    def run(self, strategies):
        ...
        with zipfile.ZipFile(self.odt_path, 'r') as yin, zipfile.ZipFile(temp_odt, 'w', zipfile.ZIP_DEFLATED) as yout:
        ...
        if modified:
            ...
            print(f"[ODT постобработка]: Закончен. Применено стратегий: {len(strategies)}")
        else:
            os.remove(temp_odt)
...
class CrossReferenceManager:
    ...
    def extract_all_links(cls, content_str: str):
        ...
        """
        cls.detected_pandoc_ids = re.findall(r'xlink:href="#([^"]*fig-[^"]+)"', content_str)
...
class FigureCaptionStrategy:
    def process(self, content_str: str) -> str:
        ...
        def find_best_matching_pandoc_id(caption_text: str) -> str:
            """
            ...
            def rough_translit(text: str) -> str:
                trans = {'щ':'shch','ш':'sh','ч':'ch','ж':'zh','х':'h','ц':'c','ю':'yu','я':'ya',
        ...
        new_content = re.sub(r'<text:p text:style-name="[^"]+">\s*</text:p>', '', new_content)

        print(f"[FigureCaptionStrategy]: Закончен. Обработано иллюстраций: {display_counter}")
        return new_content
...
class CrossReferenceStrategy:
    def process(self, content_str: str) -> str:
        ...
        pattern_link = r'(<text:a[^>]*xlink:href="#([^"]*fig-[^"]+)"[^>]*>)(.*?)(</text:a>)'

        def ref_replacer(match):
            nonlocal count
            tag_open = match.group(1)
            full_fig_id = match.group(2)
            raw_link_text = match.group(3)  # Тут может быть <text:span...>(рис. 1)</text:span>
            ...
            if ref_name:
                ...
                if digit_match:
                    ...
                    # 3. Собираем красивую обертку: текст снаружи, поле со ссылкой внутри
                    return (
        ...
        print(f"  [CrossReferenceStrategy]: Закончен. Красиво обернуто ссылок: {count} шт.")
        return content_str